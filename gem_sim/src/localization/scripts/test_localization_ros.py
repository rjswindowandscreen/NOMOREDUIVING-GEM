...
class LocalizationIntegrationTest(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info(
            f'Test started — {DRIVE_SPEED} m/s east for {DRIVE_SECONDS} s '
    ...
    def _publish_imu(self):
        ...
        self._sim_x += DRIVE_SPEED * self._sim_dt
        self._sim_y += 0.0
    ...
    def _print_gps_trace(self):
        print('\n  GPS truth vs Odom x (at each GPS update):')
        print(f'  {"t(s)":>5}  {"GPS x":>8}  {"Odom x":>8}  {"Error":>8}')
        print('  ' + '-' * 38)
        for (t, gx), (_, ox) in zip(self._gps_log, self._odom_log):
            print(f'  {t:>5.1f}  {gx:>8.2f}  {ox:>8.2f}  {ox-gx:>+8.2f}')
    ...
    def _evaluate(self, n_idle: int) -> bool:
        ...
        ok1 = n_drive > 0
        results.append((
            'Odometry messages received', ok1,
            f'{n_drive} msgs' if ok1 else 'NONE — is localization.py running?'
        ))
        if not ok1:
        ...
        results.append(('Correct frame IDs', ok2,
                         f'header={last.header.frame_id}, child={last.child_frame_id}'))

        ok3 = gx > 1.0
        results.append(('Position moved in +x', ok3,
                         f'x={gx:.2f} m, y={gy:.2f} m  (expected ~{EXPECTED_X:.1f} m)'))

        error = math.hypot(gx - EXPECTED_X, gy)
        ok4   = error < POSITION_TOL
        results.append((f'Position within {POSITION_TOL:.0f} m of truth', ok4,
                         f'error = {error:.2f} m'))

        q   = last.pose.pose.orientation
        yaw = math.atan2(2*(q.w*q.z + q.x*q.y), 1 - 2*(q.y*q.y + q.z*q.z))
        ok5 = abs(yaw) < 0.3
        results.append(('Yaw near 0° (driving east)', ok5,
                         f'yaw = {math.degrees(yaw):.1f}°'))

        vx  = last.twist.twist.linear.x
        ok6 = vx >= 0.0
        results.append(('Forward velocity vx ≥ 0', ok6, f'vx = {vx:.3f} m/s'))

        rate = n_drive / DRIVE_SECONDS
        ok7  = rate > 20
        results.append(('Publish rate ≥ 20 Hz', ok7, f'{rate:.1f} Hz'))

        ok8 = n_idle <= 5
        results.append((f'Node quiet after sensors stop', ok8,
                         f'{n_idle} msgs in {IDLE_SEC:.0f}s idle phase'))
...
def main():
    ...
    print('\nPublishing fake IMU + GPS...')
    print('Make sure localization.py is running in another terminal.\n')
